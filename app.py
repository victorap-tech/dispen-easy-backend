...
def pay_q():
    """
    Landing mínima para QR:
    /pay/q?product_id=71&litros=1   -> muestra nombre/precio y redirige a MP
    """
    pid = int(request.args.get("product_id") or 0)
    litros_req = int(request.args.get("litros") or 0)

    prod = Producto.query.get(pid)
    if not prod or not prod.habilitado:
        return "<h2>Producto no disponible</h2>", 404

    litros = litros_req if litros_req > 0 else int(getattr(prod, "porcion_litros", 1) or 1)
    backend_base = BACKEND_BASE_URL or request.url_root.rstrip("/")

    # armamos la preferencia igual que siempre
    body = {
        "items": [{
            "id": str(prod.id),
            "title": prod.nombre,
    ...
    if not MP_ACCESS_TOKEN:
        return "<h2>Configuración MP faltante</h2>", 500
    ...
    try:
        ...
        return f"<h2>Error creando preferencia</h2><pre>{str(e)[:300]}</pre>", 502

    pref = r.json() or {}
    link = pref.get("init_point") or pref.get("sandbox_init_point")
    if not link:
        return "<h2>Preferencia sin link</h2>", 502

    # HTML mínimo con nombre visible + redirección automática
    html = f"""
    <!doctype html>
    <meta charset="utf-8">
    <title>Pago · {prod.nombre}</title>
    <style>
      body{{font-family:system-ui,Segoe UI,Roboto,Arial;background:#0b1220;color:#e5e7eb;
           display:grid;place-items:center;height:100vh;margin:0}}
      .card{{background:#0f172a;border:1px solid #1f2937;border-radius:16px;padding:24px;max-width:480px}}
      .btn{{display:inline-block;background:#10b981;color:#06251d;padding:10px 14px;border-radius:10px;
           text-decoration:none;font-weight:700}}
      .sub{{opacity:.8}}
    </style>
    <div class="card">
      <h2 style="margin:0 0 8px">Pagar: {prod.nombre}</h2>
      <p class="sub">Precio por L: ${prod.precio:.2f} · Porción: {litros} L</p>
      <p>Te vamos a redirigir al checkout de Mercado Pago…</p>
      <p><a class="btn" href="{link}">Ir a pagar</a></p>
    </div>
    <script>setTimeout(()=>location.href={_json.dumps(link)},1200)</script>
    """
    return html, 200
# -------------------------------------------------------------
